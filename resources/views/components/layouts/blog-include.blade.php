...
<section class=" w-full flex flex-col bg-tradewind-100 p-10">
    <div class="px-10 mx-auto max-w-7xl w-full">
        <div class="flex items-center justify-between w-full pb-5 mb-8">
            <h1
                class="w-full m-0 text-4xl font-black leading-loose tracking-wide text-center text-rolling-stone-700 border-0 border-rolling-stone-200 sm:text-5xl">
                Blog Posts
            </h1>
        </div>
            ...
            @foreach ($posts as $post)
                <div class="relative col-span-12 mb-10 space-y-4 md:col-span-6 lg:col-span-4">
                    ...
                    <a href=wire:navigate href="{{ route('blog.show', $post->slug) }}"
                        class="block text-2xl font-medium leading-tight text-rolling-stone-700 hover:text-rolling-stone-900">{{ $post->title }}</a>
                </div>
            @endforeach
        </div>
        <div class="flex justify-center">
            <a wire:navigate href="{{ route('blog.index') }}"
                class="flex items-center rounded font-semibold text-rolling-stone-800 hover:text-rolling-stone-950">
